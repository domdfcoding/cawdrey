...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive cawdrey/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive cawdrey/
  >&2 flake8 --select "$errors$belligerent" cawdrey/
